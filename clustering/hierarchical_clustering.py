...
def hierarchical_clustering(X_pca, y):
  # Prepare data
  ...
  for n_clusters in clusters_range:
    ...
    cluster_labels = hierarchical.fit_predict(X_pca)

    silhoutte = silhouette_score(X_pca, cluster_labels)
  ...
  cluster_labels_best = hierarchical_best.fit_predict(X_pca)
  ...
  print(f"Hierarchical clustering with Ward linkage fits this obesity dataset well because the points are grouped based on distances in the features and doesn't really need class labels to find structure. From the k values tested, k={best_n_clusters} achieved the highest silhouette score of {best_sillhouette:.3f} compared to other k values tested. Therefore, the algorithm and the PCA successfully identified {best_n_clusters} clusters that fit the major obesity categories. \n")
